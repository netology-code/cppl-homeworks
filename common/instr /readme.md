...
Важно отметить, что тип аргумента в виртуальной функции some_func из базового класса - int. А тип аргумента в функции some_func из производнного класса - double. Поэтому функция some_func в производном классе не является переопределением виртуальной функции some_func из базового класса.

Поэтому выводится значение 2.0 "in derived class". Затем объект удаляется оператором delete для предотвращения утечки памяти.
