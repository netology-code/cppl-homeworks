...
Начиная со стандарта С++14 возвращаемым типом функции может быть auto.
...
Есть входная переменная типа int. 
Компилятор понимает, что выходным значением будет тоже int.
Важно, чтобы первая точка возхврата return говорила, какой возвращаемый тип у функции.
...
Здесь первый return возвращает рекурсию. Компилятору это ни о чём не говорит.
...
Однако вложенные структуры так не получится декомпозировать.
