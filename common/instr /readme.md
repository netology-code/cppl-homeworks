...
Если раньше мы были ограничены правилами C++, что массив типа int содержит элементы типа int, массив типа float содержит элементы типа float и т.д., то с появлением кортежей в стандарте C++11 мы можем хранить элементы различных типов в одной коллекции.

В C++ кортежи реализуются с помощью структуры `std::tuple`. 

Синтаксис кортежа в C++ следующий:

- Вариант 1: std::tuple<std::string, int, char, double> test(“str”, 4, ‘a’, 3.0)// Создание кортежа через конструктор

- Вариант 2: auto simple_tuple = std::make_tuple(“str”, 4, ‘a’, 3.0) // Создание с помощью функции make_tuple
