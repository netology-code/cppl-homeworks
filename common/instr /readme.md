...
Для того, чтобы понять суть семантики перемещения, представьте, что у вас есть пустое новое ведро и старое ведро, наполненное апельсинами.
Ведро с апельсинами покрыто трещинами. Его пора выбросить.
Но мы не будем выбрасывать вместе с ним апельсины и покупать новые.
Мы переместим апельсины из старого ведра в новое.

Рассмотрим пример с кодом:

![image](https://github.com/netology-code/cppl-homeworks/assets/147130852/84a1485c-e3d7-4a87-8c46-f236139d6822)

У нас есть функция void swap, которая принимает 2 массива по ссылке и меняет их местами:

Внутри функции создается временный вектор tmp, который принимает значения вектора lhs. 
Затем значение вектора lhs заменяется значениями вектора rhs, а значение вектора rhs заменяется значениями вектора tmp.

Здесь произойдёт три копирования, три потенциально очень затратных операции.
