...
1. Есть структура `exam_results`, которая представляет собой дневник с оценками учеников.
...
Спецификатор `mutabl`e позволяет изменять значения переменных, захваченных лямбда-функцией по значению. Однако, важно понимать, что даже если `val` изменяется внутри лямбда-функции, это не влияет на исходную переменную `val`, определенную вне лямбда-функции.
