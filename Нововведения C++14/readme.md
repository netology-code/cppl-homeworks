...
`auto tmp = std::vector<int>(100000, 0);` // Объект std::vector<int> с именем tmp, содержит 100000 элементов, все инициализированны значением 0


`auto func = [val = std::move(tmp)]{ return val[0]; };` // Создается лямбда-функция func, которая захватывает tmp с помощью перемещения (используя std::move). Лямбда-функция возвращает значение первого элемента вектора tmp (val[0]).
